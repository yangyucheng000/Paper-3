...
################################################################################
@click.command()
#  type=click.Choice(['mnist_LeNet', 'cifar10_LeNet', 'cifar10_LeNet_ELU']),
@click.argument('dataset_name', type=click.Choice(['mnist', 'cifar10']), default='cifar10')
@click.argument('net_name', default='cifar10_LeNet')
@click.argument('xp_path', type=click.Path(exists=True), default='../log/')
# @click.argument('data_path', type=click.Path(exists=True))
@click.argument('data_path', type=click.Path(exists=True), default='E://dataset//cifar-10-python')
...
@click.option('--lr_milestone', type=int, default=(1,), multiple=True,
...
@click.option('--ae_lr_milestone', type=int, default=(1,), multiple=True,
              help='Lr scheduler milestones at which lr is multiplied by 0.1. Can be multiple and must be increasing.')
...
def main(dataset_name, net_name, xp_path, data_path, load_config, load_model, objective, nu, device, seed,
    ...
    logger = logging.getLogger() # 默认root
    ...
    # train_loader, test_loader = dataset.loader(batch_size=32)
    ...
    if pretrain:
        ...
        # Pretrain model on dataset (via autoencoder)，并且使用autoencoder的encoder初始化网络
    ...
    # Test model
    deep_SVDD.test(dataset, device=device, n_jobs_dataloader=n_jobs_dataloader, ema=ema)
