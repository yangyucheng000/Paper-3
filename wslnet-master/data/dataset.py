...
class Data():
    def __init__(self, cfg):
        name = cfg.mode.split('_')
        with open(cfg.datapath + '/' + name[0] + '.txt', 'r') as lines:
            self.samples = []
            if len(name) == 1:
                if name[0] == 'test' or name[0] == 'test0':
                    for line in lines:
                        imagepath = cfg.datapath + '/image/' + line.strip() + '.jpg'
                        maskpath = cfg.datapath + '/mask/' + line.strip() + '.png'
                        coarsepath = cfg.datapath + '/mask/' + line.strip() + '.png'
                        self.samples.append([imagepath, maskpath, coarsepath])

                else:
                    for line in lines:
                        imagepath = cfg.datapath + '/image/' + line.strip() + '.jpg'
                        maskpath = cfg.datapath + '/mask/' + line.strip() + '.png'
                        coarsepath = cfg.datapath + '/coarse/' + line.strip() + '_RBD.png'
                        self.samples.append([imagepath, maskpath, coarsepath])
            else:
                n = 1
                for line in lines:
                    for num in name[1:]:
                        ...
                        if n >= num * 1000 and n <= (num + 1) * 1000:
                            imagepath = cfg.datapath + '/image/' + line.strip() + '.jpg'
                            if num == 0:
                                maskpath = cfg.datapath + '/mask/' + line.strip() + '.png'
                                msg = 'gt mask, num=%s' % (maskpath)
                                logger.info(msg)
                            else:
                                maskpath = cfg.datapath + '/coarse/' + line.strip() + '_RBD.png'
                                msg = 'coarse mask, num=%s' % (maskpath)
                                logger.info(msg)

                            coarsepath = './data/coarse/' + line.strip() + '_RBD.png'
                            self.samples.append([imagepath, maskpath, coarsepath])
                    n += 1
        ...
        if name[0] == 'train':
            ...
            self.transform = transform.Compose(transform.Normalize(mean=cfg.mean, std=cfg.std),
                                               ...
                                               # vision.RandomRotate(),
                                               transform.ToTensor())
        elif name[0] == 'test' or name[0] == 'test0':
            self.transform = transform.Compose(transform.Normalize(mean=cfg.mean, std=cfg.std),
                                               transform.Resize(320, 320),
                                               transform.ToTensor())
    ...
    def __getitem__(self, idx):
        imagepath, maskpath, coarsepath = self.samples[idx]
        image = cv2.imread(imagepath).astype(np.float32)[:, :, ::-1]
        mask = cv2.imread(maskpath).astype(np.float32)[:, :, ::-1]
        coarse = cv2.imread(coarsepath).astype(np.float32)[:, :, ::-1]

        H, W, C = mask.shape

        image, mask, coarse = self.transform(image, mask, coarse)
        return image, mask, coarse, H, W, maskpath.split('/')[-1]
        #
