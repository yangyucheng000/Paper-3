...
from .layers import to_2tuple, DropPath
import math
...
class Attention(nn.Cell):
    ...
    def construct(self, x, mask=None, flag=False):
        """
        Args:
        ...
        if True:
            ...
            query_layer = q  # self.transpose_for_scores(q)
            key_layer = k  # self.transpose_for_scores(k)
            value_layer = v  # self.transpose_for_scores(v)

            attention_scores = ops.matmul(query_layer, key_layer.transpose(0, 1, 3, 2))
            attention_scores = attention_scores / math.sqrt(32)
            ...
            new_context_layer_shape = context_layer.shape[:-2] + (self.dim,)
...
class CrossFormerBlock(nn.Cell):
    ...
    def construct(self, x, flag=False):
        ...
        if self.lsda_flag == 0:  # 0 for SDA
            ...
            x = x.reshape(B, G, H // G, G, W // G, C).permute(0, 2, 4, 1, 3, 5) # 2 8 8 7 7 96
        x = x.reshape(B * H * W // G ** 2, G ** 2, C) # 128 49 96

        # multi-head self-attention
        x,wg = self.attn(x, mask=self.attn_mask, flag=flag)  # nW*B, G*G, C 128 49 96
        # ungroup embeddings
        x = x.reshape(B, H // G, W // G, G, G, C) # 2 8 8 7 7 96
        if self.lsda_flag == 0:
            x = x.permute(0, 1, 3, 2, 4, 5).reshape(B, H, W, C)
        else:
            x = x.permute(0, 3, 1, 4, 2, 5).reshape(B, H, W, C) # 2 56 56 96
        x = x.view(B, H * W, C)

        # FFN
        x = shortcut + self.drop_path(x) # 2 3136 96 
...
class CrossFormer(nn.Cell):
    ...
    def __init__(self, hash_bit=64, label=13, img_size=224, patch_size=[4, 8, 16, 32], in_chans=3, num_classes=1000,
                 embed_dim=96, depths=[2, 2, 2], num_heads=[3, 6, 12, 24],
    ...
    def construct_features(self, x):
        ...
        x, wg = self.layers[-1](x, True)
        for i in wg:
            weg.append(i.view(-1, 12 * 4, i.shape[2], i.shape[3]))


        part_num, part_inx = self.part_select(weg)
        ...
        parts = ops.stack(parts)

        x = ops.cat((x[:, 0].unsqueeze(1), parts), axis=1)
...
class Part_Attention(nn.Cell):
    ...
    def construct(self, x):
        ...
        for i in range(1, length):
            last_map = ops.matmul(x[i], last_map)

        last_map = last_map[:, :, 0, 1:]
        _, max_inx = last_map.max(axis=2, return_indices=True)
...
if __name__ == '__main__':
    ...
    x = ms.ops.randn((1, 3, 224, 224))
    model = CrossFormer(num_classes=13)
    output = model(x)
    print(output.shape)
    # 60, 256