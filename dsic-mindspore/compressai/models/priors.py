...
class CompressionModel(nn.Cell):
    ...
    ### pytorch的module.chidren和mindspore的Cells.cells()对应，module.mosules()使用Cell.cells_and_names()替换
    def aux_loss(self):
        """Return the aggregated loss over the auxiliary entropy bottleneck
        module(s).
        """
        aux_loss = sum(m.loss() for (_, m) in self.cells_and_names()
    ...
    def _initialize_weights(self):
        for (name, cell) in self.cells_and_names():
            if isinstance(cell, (nn.Conv2d, nn.Conv2dTranspose)):
                cell.weight.set_data(ms.common.initializer.initializer('he_normal', cell.weight.shape, cell.weight.dtype))
    ...
    def update(self, force=False):
        ...
        for m in self.cells():
            if not isinstance(m, EntropyBottleneck):
                continue
            m.update(force=force) ### update可能是EntropyBottleneck类的属性，暂不修改
...
class FactorizedPrior(CompressionModel):
    ...
    def construct(self, x):
        ...
        return x_hat,y_likelihoods   #不能以字典的形式输出，否则无法计算梯度
        '''
        return {
            'x_hat': x_hat,
            'likelihoods': {
...
def get_scale_table(min=SCALES_MIN, max=SCALES_MAX, levels=SCALES_LEVELS):  # pylint: disable=W0622
    return ops.exp(ops.LinSpace(math.log(min), math.log(max), levels))
...
class ScaleHyperprior(CompressionModel):
    ...
    def construct(self, x):
        ...
        return x_hat,y_likelihoods,z_likelihoods   #不能以字典的形式输出，否则无法计算梯度
    ...
    def decompress(self, strings, shape):
        ...
        x_hat = ms.ops.clip_by_value(self.g_s(y_hat), clip_value_min=0, clip_value_max=1) #通过g_s网络，获得估计图像
...
class MeanScaleHyperprior(ScaleHyperprior):
    ...
    def construct(self, x):
        ...
        scales_hat, means_hat = ops.split(gaussian_params, 2, 1)
