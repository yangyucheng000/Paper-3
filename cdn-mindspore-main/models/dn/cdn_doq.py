...
class CDN(nn.Cell):
    ...
    def construct(self, src, mask, query_embed, query_verb_embed, pos_embed, tgt_padding_mask=None, attn_mask=None):
        bs, c, h, w = src.shape
        ...
        if attn_mask is not None:
            ...
            query_verb_embed = query_verb_embed.unsqueeze(1).tile((1, bs, 1))
        else:
        ...
        # todo add attn mask and padding mask
        # query_verb_embed = F.pad(query_verb_embed.permute(2,1,0), pad=(0,interaction_query_embed.shape[0] - query_verb_embed.shape[0],0,0), mode="constant",value=0).permute(2,1,0)
        interaction_decoder_out = self.interaction_decoder(interaction_tgt, memory, tgt_mask=attn_mask, tgt_key_padding_mask=tgt_padding_mask,
