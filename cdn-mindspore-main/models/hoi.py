...
class CDNHOI(nn.Cell):
    ...
    def construct(self, x, m):
        ...
        hopd_out, interaction_decoder_out = self.transformer(self.input_proj(src), mask, self.query_embed.embedding_table, pos[-1])[:2]
...
class PostProcessHOI(object):
    ...
    def __call__(self, outputs, target_sizes):
        ...
        scale_fct = ops.stack([img_w, img_h, img_w, img_h], dim=1).to(verb_scores.device)
