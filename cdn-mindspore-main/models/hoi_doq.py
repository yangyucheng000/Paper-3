...
from models.dn.cdn_doq import build_cdn
from models.dn.doq_components import prepare_for_doq, doq_post_process
...
class CDNHOI(nn.Cell):
    ...
    def __init__(self, backbone, transformer, num_obj_classes, num_verb_classes, num_queries, aux_loss=False, args=None):
        ...
        # add verb query
        self.query_verb_embed = nn.Embedding(num_queries, hidden_dim)
    ...
    def construct(self, x, m, doq_args=None):
        src, mask, pos = self.backbone(x, m)
        assert mask is not None

        if doq_args is not None:
            # todo prepare for doq
            input_query_with_gt_position_query, attn_mask, tgt_padding_mask, mask_dict = \
                prepare_for_doq(doq_args, src.shape[0], self.bbox_enc, self.query_embed.embedding_table)
            ####
            hopd_out, interaction_decoder_out = self.transformer(self.input_proj(src), mask,
                                        input_query_with_gt_position_query, self.query_verb_embed.embedding_table, pos[-1], None, attn_mask)[:2]
            ...
            hopd_out, interaction_decoder_out = self.transformer(self.input_proj(src), mask, self.query_embed.embedding_table, self.query_verb_embed.embedding_table,
                                                                 pos[-1])[:2]
